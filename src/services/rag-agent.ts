	...
	private async checkRelevance(query: string): Promise<{ isRelevant: boolean }> {
		...
		try {
			...
			const response = await fetch(this.baseUrl, {
				method: 'POST',
				headers: {
					Authorization: `Bearer ${String(this.openRouterKey)}`,
			...
			const similarConversations = await db
				...
				.where(
					and(
						...
						gt(sql`1 - (${cosineDistance(embeddings.embedding, queryEmbedding)})`, 0.7),
					),
				)
				.orderBy(desc(sql`1 - (${cosineDistance(embeddings.embedding, queryEmbedding)})`))
				.limit(3)

			console.log(`📋 Found ${String(similarConversations.length)} relevant conversations`)

			if (similarConversations.length > 0) {
				console.log('🎯 Context matches:')
				similarConversations.forEach((item, index) => {
					console.log(
						`  ${String(index + 1)}. Similarity: ${item.similarity?.toFixed(3) ?? '0.000'} - "${item.content.substring(0, 60)}..."`,
					)
				})
			} else {
