...
export async function POST(request: Request) {
    try {
        ...
        let jobTitle: string | undefined = body.job_title

        // Check if we have a URL that might be a Greenhouse job posting
        if (body.url && typeof body.url === 'string') {
            try {
                const jsonUrl = turnUrlToJsonUrl(body.url)
                if (jsonUrl) {
                    ...
                    jobDescription = extractAndClean(data)
                    // Use the title from Greenhouse if available
                    jobTitle = data.title || jobTitle
                } else {
                    throw new Error('Not a valid Greenhouse URL')
                }
            } catch (error) {
                console.error('Error processing Greenhouse URL:', error)
                return NextResponse.json(
                    { error: 'Failed to process job URL' },
                    { status: 400 }
                )
            }
        } else if (body.job_description) {
            jobDescription = body.job_description
        } else {
            return NextResponse.json(
                { error: 'Either job_description or a valid Greenhouse URL is required' },
                { status: 400 }
            )
        }
