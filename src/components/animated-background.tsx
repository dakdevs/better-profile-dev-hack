	...
	useEffect(() => {
		...
		const animate = () => {
			...
			particlesRef.current.forEach((particle, _index) => {
				...
				ctx.fillStyle = `hsla(${String(currentHue)}, 45%, 45%, ${String(currentOpacity * 0.12)})`
				ctx.fill()

				ctx.beginPath()
				ctx.arc(particle.x, particle.y, particle.size * 1.2, 0, Math.PI * 2)
				ctx.fillStyle = `hsla(${String(currentHue)}, 55%, 35%, ${String(currentOpacity * 0.25)})`
				ctx.fill()

				ctx.beginPath()
				ctx.arc(particle.x, particle.y, particle.size * 0.8, 0, Math.PI * 2)
				ctx.fillStyle = `hsla(${String(currentHue)}, 65%, 25%, ${String(currentOpacity * 0.8)})`
				ctx.fill()
