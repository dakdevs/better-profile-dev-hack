...
function stripHtmlTags(html: string): string {
	...
	cleanText = cleanText
		...
		.replace(/&deg;/g, '°').replace(/&plusmn;/g, '±').replace(/&times;/g, '×')
		.replace(/&divide;/g, '÷');
...
function extractAndClean(data: any): string {
	...
	const header = [title, company ? `Company: ${company}` : '', location ? `Location: ${location}` : ''].filter(Boolean).join('\n');
	...
	importFromURL: protectedBase
		...
		.handler(async function({ input }) {
			try {
				...
				console.error('[orpc/importFromURL] Error:', error);
