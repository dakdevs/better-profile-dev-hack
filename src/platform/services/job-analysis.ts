...
import { CircuitBreaker, withRetry } from '~/lib/error-handler'
import { AIProcessingError, ExternalServiceError, ValidationError } from '~/lib/errors'
import { logger, withLogging } from '~/lib/logger'
import { batchUtils, rateLimit, rateLimiters } from '~/lib/rate-limiter'
import {
	ExperienceLevel,
	ExtractedSkill,
	JobAnalysisResult,
...
export class JobAnalysisService {
	...
	async analyzeJobPosting(
		...
		if (limitInfo.isBlocked) {
			throw new Error(
				`Rate limit exceeded for AI analysis. Try again in ${Math.ceil(limitInfo.msBeforeNext / 1000)} seconds.`,
			)
		}
		// Validate input
		if (
		...
		if (jobDescription.length > 50000) {
			throw new ValidationError(
				'Job description is too long (max 50,000 characters)',
				'jobDescription',
		...
		return withLogging('job-analysis.analyze', async () => {
			...
			try {
				...
				const response = await this.circuitBreaker.execute(() => this.callAIWithRetry(prompt))
				...
				logger.error(
					'Unexpected error in job analysis, using fallback',
					{
						operation: 'job-analysis.analyze',
					},
					aiError,
			...
			async () => {
				...
				try {
					...
					if (!data.choices || !data.choices[0] || !data.choices[0].message) {
						throw new AIProcessingError('Invalid AI API response structure', undefined, false)
					}
	...
	private parseAIResponse(response: string): Partial<JobAnalysisResult> {
		...
		try {
			...
			if (error instanceof SyntaxError) {
				throw new AIProcessingError('AI returned invalid JSON format', undefined, true, error)
			}
	...
	private async validateAndEnhanceResult(
		...
		if (result.extractedSkills.length === 0 || result.confidence < 0.3) {
			...
			result.extractedSkills =
				result.extractedSkills.length > 0 ? result.extractedSkills : fallback.extractedSkills
