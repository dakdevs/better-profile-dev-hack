...
import { ScoringError, ValidationError, ValidationUtils } from './ValidationUtils'
...
export class ScoringEngine implements IScoringEngine {
		...
		try {
			if (!strategy || typeof strategy.calculateScore !== 'function') {
				throw new ValidationError(
					'Strategy must implement calculateScore method',
					'strategy',
					strategy,
				)
			}
	...
	async calculateScore(qaPair: QAPair, context: ScoringContext): Promise<number> {
		...
		try {
			...
			console.warn(
				'Primary scoring strategy failed, trying fallback:',
			...
			try {
				...
				console.error(
					'Fallback scoring strategy also failed:',
					ValidationUtils.createSafeErrorMessage(fallbackError as Error),
				)
				...
				try {
					...
					console.error(
						'All scoring methods failed, using neutral score:',
						ValidationUtils.createSafeErrorMessage(simpleError as Error),
					)
					return 50
				}
			}
