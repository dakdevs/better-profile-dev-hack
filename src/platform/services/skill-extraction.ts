...
export class SkillExtractionService {
	...
	async extractSkills(
		text: string,
		context?: 'job_posting' | 'interview' | 'resume',
		...
		return withLogging('skill-extraction.extract', async () => {
			...
			if (serverConfig.ai.openRouterApiKey) {
				try {
					...
					logger.warn(
						'AI skill extraction failed, using hybrid approach',
						{
	...
	private async extractWithHybrid(text: string, context?: string): Promise<SkillExtractionResult> {
		...
		const sortedSkills = uniqueSkills.sort((a, b) => b.confidence - a.confidence).slice(0, 50) // Limit to top 50 skills
	...
	private extractDomainSpecificSkills(text: string): ExtractedSkill[] {
		...
		// Healthcare/Medical
		if (
			this.containsDomainKeywords(text, [
				'medical',
				'healthcare',
				'clinical',
				'patient',
...
Return ONLY the JSON object, no additional text.
	...
	private parseAISkillResponse(response: string, originalText: string): SkillExtractionResult {
		try {
			...
			logger.error(
				'Failed to parse AI skill extraction response',
				{
