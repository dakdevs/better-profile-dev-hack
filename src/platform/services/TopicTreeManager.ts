...
import { TreeIntegrityError, ValidationError, ValidationUtils } from './ValidationUtils'
...
export class TopicTreeManager implements ITopicTreeManager {
	...
	addNode(node: ITopicNode): void {
		try {
			...
			if (this._tree.nodes.has(topicNode.id)) {
				throw new ValidationError(
					`Node with ID ${topicNode.id} already exists`,
					'nodeId',
					topicNode.id,
				)
			}
			...
			if (topicNode.parentTopic) {
				...
				if (!parentNode) {
					...
					throw new ValidationError(
						`Parent node ${topicNode.parentTopic} not found`,
						'parentTopic',
						topicNode.parentTopic,
					)
				}
	...
	findDeepestUnvisitedBranch(): TopicNode | null {
		...
		const unvisitedLeaves = leafNodes.filter(
			(node) => !node.metadata.isExhausted && node.metadata.visitCount === 0,
		)
	...
	getTree(): ConversationTree {
		...
		return {
			nodes: new Map(
				Array.from(this._tree.nodes.entries()).map(([id, node]) => [id, node.toPlainObject()]),
			),
	...
	private _validateTreeIntegrity(): void {
		...
		for (const [nodeId, node] of this._tree.nodes) {
			if (node.parentTopic) {
				...
				if (!parent) {
					throw new Error(
						`Tree integrity violation: node ${nodeId} has invalid parent ${node.parentTopic}`,
					)
				}
				if (!parent.children.includes(nodeId)) {
					throw new Error(
						`Tree integrity violation: parent ${node.parentTopic} doesn't list ${nodeId} as child`,
					)
				}
			}
		}
		...
		for (const [nodeId, node] of this._tree.nodes) {
			if (this._hasCircularReference(node, new Set())) {
				throw new Error(
					`Tree integrity violation: circular reference detected starting from ${nodeId}`,
				)
			}
		}
	}
