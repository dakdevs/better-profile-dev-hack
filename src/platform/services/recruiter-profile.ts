...
export class RecruiterProfileService {
	...
	async createProfile(
		...
		console.log('[RECRUITER-PROFILE-SERVICE] Creating profile for user:', userId, 'with data:', {
		...
		try {
			...
			if (dbError instanceof Error && dbError.message.includes('foreign key')) {
				console.log(
					'[RECRUITER-PROFILE-SERVICE] Foreign key constraint failed - user does not exist',
				)
	...
	async updateProfile(
		...
		// Update profile
		await db.update(recruiterProfiles).set(updateData).where(eq(recruiterProfiles.userId, userId))
	...
	private validateBusinessRules(
		data: CreateRecruiterProfileRequest | UpdateRecruiterProfileRequest,
	): void {
	...
	async getProfileWithUser(
		userId: string,
	): Promise<(RecruiterProfile & { user: { name: string; email: string } }) | null> {
