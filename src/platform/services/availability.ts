...
export class AvailabilityService {
	...
	async createAvailability(
		...
		if (conflicts.length > 0) {
			throw new Error(
				`Availability conflicts with existing slots: ${conflicts.map((c) => c.description).join(', ')}`,
			)
		}
	...
	async getCandidateAvailability(
		...
		let query = db
			...
			.where(eq(candidateAvailability.userId, userId))
	...
	async updateAvailability(
		...
		if (updateData.startTime || updateData.endTime) {
			...
			if (conflicts.length > 0) {
				throw new Error(
					`Updated availability conflicts with existing slots: ${conflicts.map((c) => c.description).join(', ')}`,
				)
			}
		}
	...
	async deleteAvailability(
		...
		await db.delete(candidateAvailability).where(eq(candidateAvailability.id, availabilityId))
