...
import { ScoringError, TreeIntegrityError, ValidationError } from '../ValidationUtils'
...
describe('ConversationGradingSystem Error Handling', () => {
	...
	describe('Scoring Error Handling', () => {
		...
		it('should validate scoring strategy on set', () => {
			...
			expect(() => system.setScoringStrategy({ notCalculateScore: () => {} } as any)).toThrow(
				ValidationError,
			)
		})
		...
		it('should validate topic analyzer on set', () => {
			...
			expect(() => system.setTopicAnalyzer({ extractTopics: () => {} } as any)).toThrow(
				ValidationError,
			)
		})
	})
