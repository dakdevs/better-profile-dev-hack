...
describe('TopicAnalyzer', () => {
	...
	describe('extractTopics', () => {
		...
		it('should handle Q&A pairs with technical content', async () => {
			const qaPair: QAPair = {
				...
				answer:
					'Backpropagation calculates gradients by applying the chain rule through network layers.',
				timestamp: new Date(),
			}
	...
	describe('determineRelationship', () => {
		...
		it('should detect child relationship for similar topics', () => {
			const existingNodes = [createMockNode('1', 'machine learning', null)]

			const relationship = analyzer.determineRelationship(
		...
		it('should detect continuation with various keywords', () => {
			const existingNodes = [createMockNode('1', 'deep learning', null)]
			existingNodes[0].updatedAt = new Date()
