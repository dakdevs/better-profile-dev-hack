	...
	describe('QualityScoringStrategy', () => {
		...
		it('should give higher scores for answers with reasoning', async () => {
			const reasoningAnswer: QAPair = {
				...
				answer:
					'This is important because it provides clear structure and therefore improves maintainability.',
				timestamp: new Date(),
			}
		...
		it('should reward detailed answers with examples', async () => {
			const detailedAnswer: QAPair = {
				...
				answer:
					'This works by following a specific pattern. For example, when you call the method, it first validates the input and then processes it accordingly.',
				timestamp: new Date(),
			}
	...
	describe('ComplexityScoringStrategy', () => {
		...
		it('should reward technical terminology', async () => {
			const technicalAnswer: QAPair = {
				...
				answer:
					'The implementation uses a specific algorithm and follows architectural patterns within the framework.',
				timestamp: new Date(),
			}
	...
	describe('ContextAwareScoringStrategy', () => {
		...
		it('should reward answers that reference previous conversation', async () => {
			...
			const referencingAnswer: QAPair = {
				...
				answer:
					'As mentioned previously, TypeScript provides type safety which helps catch errors.',
				timestamp: new Date(),
			}
			...
			const nonReferencingScore = await strategy.calculateScore(
				nonReferencingAnswer,
				contextWithHistory,
			)
	...
	describe('WeightedScoringStrategy', () => {
		...
		it('should combine multiple scoring factors', async () => {
			const comprehensiveAnswer: QAPair = {
				...
				answer:
					'The architecture follows a layered pattern because it provides separation of concerns. For example, the data layer handles persistence while the business layer manages logic.',
				timestamp: new Date(),
			}
