...
export class JobPostingService {
	...
	async createJobPosting(
		...
		try {
			...
			let analysis
			try {
				analysis = await jobAnalysisService.analyzeJobPosting(data.description, data.title)
				...
				console.error(
					'[JOB-POSTING-SERVICE] Error has fallbackData:',
					!!(analysisError as any)?.fallbackData,
				)
					...
					analysis = {
						...
						salaryRange:
							data.salaryMin && data.salaryMax
								? { min: data.salaryMin, max: data.salaryMax }
								: undefined,
						keyTerms: [],
						confidence: 0.3,
			...
			let createdJob
			try {
				;[createdJob] = await db.insert(jobPostings).values(jobData).returning()
	...
	async getJobPosting(jobId: string, recruiterId?: string): Promise<JobPostingResponse> {
		try {
			const query = db.select().from(jobPostings).where(eq(jobPostings.id, jobId))

			// If recruiterId is provided, ensure the job belongs to the recruiter
			if (recruiterId) {
				query.where(and(eq(jobPostings.id, jobId), eq(jobPostings.recruiterId, recruiterId)))
			}
	...
	async getJobPostings(
		...
		try {
			...
			const allJobsCount = await db.select({ count: sql<number>`count(*)` }).from(jobPostings)
			...
			console.log(
				'[JOB-POSTING-SERVICE] Jobs for recruiter',
				recruiterId,
				':',
				recruiterJobsCount[0]?.count || 0,
			)
			...
			console.log(
				'[JOB-POSTING-SERVICE] Executing select query with limit:',
				limit,
				'offset:',
				offset,
			)
		...
		try {
			...
			if (data.description !== undefined) {
				...
				try {
					...
					console.warn(
						'AI analysis failed during update, continuing without re-analysis:',
						analysisError,
					)
				}
			}
			...
			const [updatedJob] = await db
				.update(jobPostings)
				.set(updateData)
				.where(and(eq(jobPostings.id, jobId), eq(jobPostings.recruiterId, recruiterId)))
				.returning()
