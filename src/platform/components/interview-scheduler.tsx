...
export function InterviewScheduler({
	...
	const fetchAvailableSlots = async (date: string) => {
		...
		try {
			...
			console.log('[INTERVIEW_SCHEDULER] Fetching slots for:', {
				eventTypeId,
				date,
				startTime,
				endTime,
			...
			if (!response.ok) {
				...
				throw new Error(
					errorData.error || `HTTP ${response.status}: Failed to fetch available time slots`,
				)
			}
	...
	return (
		<div className="mx-auto max-w-2xl rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-black">
			<h3 className="mb-6 text-xl font-semibold text-black dark:text-white">Schedule Interview</h3>
			...
			<div className="space-y-6">
							...
							<div className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4">
								{availableSlots.map((slot, index) => (
									<button
										key={index}
										onClick={() =>
											setSelectedTime(new Date(slot.time).toTimeString().split(' ')[0])
										}
				...
				{/* Error Display */}
				{error && (
					<div className="bg-apple-red/10 border-apple-red/20 rounded-lg border p-3">
				...
				<div className="flex gap-3 pt-4">
					<Button
						...
						className="bg-apple-blue flex-1 text-white hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-60"
					>
