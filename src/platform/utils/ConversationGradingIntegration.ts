...
import { ConversationGradingSystem, ConversationGradingSystemWithSessions } from '../services'
import {
	ConversationTree,
	IScoringStrategy,
	ITopicAnalyzer,
...
export class ConversationGradingIntegration {
	...
	constructor(config: IntegrationConfig = {}) {
		...
		if (this.config.enableSessions) {
			this.system = new ConversationGradingSystemWithSessions(this.config.persistenceConfig || {})
		} else {
	...
	async processQAPair(qaPair: QAPair): Promise<ProcessingResult> {
		...
		const nodeId = await this.system.addQAPair(qaPair, this.config.autoScore ? undefined : null)
	...
	getAnalytics(): {
		...
		const topicStats = new Map<
			string,
			{ qaPairCount: number; totalDepth: number; scores: number[] }
		>()
...
export function createSimpleTopicAnalyzer(
	...
	return {
		...
		determineRelationship(topic: string, existingNodes: TopicNode[]): TopicRelationship {
			...
			const similarNode = existingNodes.find(
				(node) =>
