...
export function withAPISecurity(config: APISecurityConfig = {}) {
  ...
  return function <T extends (...args: any[]) => any>(
    ...
    descriptor.value = withErrorHandling(async function (request: NextRequest, ...args: any[]) {
      ...
      const result = await originalMethod.call(this as any, request, { user, ...args });
...
export function createSecureAPIResponse<T>(
  ...
  const secureResponse = NextResponse.json(response, { status });
  const securityHeaders = RequestSecurity.getSecurityHeaders();
