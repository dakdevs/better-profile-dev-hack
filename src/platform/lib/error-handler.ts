...
export function handleApiError(
	...
	if (error instanceof BaseError) {
		...
		if (logLevel === 'error') {
			logger.error(
				'Operational error in API request',
				{
					requestId,
					operation: `${requestContext.method} ${requestContext.url}`,
...
export async function withServiceErrorHandling<T>(
	...
	try {
		...
		if (error instanceof Error && error.message.includes('database')) {
			const dbError = new DatabaseError('Database operation failed', operation, undefined, error)
...
export async function withRetry<T>(
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		try {
			...
			logger.warn(
				`Operation failed, retrying in ${currentDelay}ms (attempt ${attempt}/${maxAttempts})`,
				{
					operation: 'retry',
