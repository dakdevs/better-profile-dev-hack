...
export class PaginationHelper {
	...
	createResult<T>(data: T[], total: number, page: number, limit: number): PaginationResult<T> {
...
export class SearchHelper {
	...
	static generateSearchSQL(
		...
		return conditions.length > 0 ? sql`(${sql.join(conditions, sql` OR `)})` : sql`TRUE`
	}

	// Generate filter SQL
	static generateFilterSQL(filters: Record<string, any>, tableColumns: Record<string, any>): SQL {
		const conditions: SQL[] = []
	...
	static generateDateRangeSQL(dateRange: { start?: Date; end?: Date }, dateColumn: any): SQL {
		const conditions: SQL[] = []
...
export class LazyLoader<T> {
	...
	constructor(loadFn: (offset: number, limit: number) => Promise<T[]>, batchSize: number = 50) {
		this.loadFn = loadFn
	...
	async loadRange(startIndex: number, endIndex: number): Promise<T[]> {
		...
		const batches = await Promise.all(
			Array.from({ length: endBatch - startBatch + 1 }, (_, i) => this.loadBatch(startBatch + i)),
		)
