...
export function useFormValidation<T>(schema: z.ZodSchema<T>) {
	return {
		...
		validateField: (fieldName: string, value: unknown) => validateField(schema, fieldName, value),
	}
}
 ...
 */
export function validateSalaryRange(minSalary?: number, maxSalary?: number): FieldValidationResult {
...
export function validateFile(
	...
	if (!file) {
		return required ? { isValid: false, error: 'File is required' } : { isValid: true }
	}
