...
export const sanitizedTextSchema = (maxLength: number = 1000) =>
	z
		...
		.refine(
			(text) => InputSanitizer.validateSQLSafety(text),
...
export const jobPostingCreateSchema = z
	.object({
		...
		requiredSkills: z
			.array(sanitizedTextSchema(100))
			.max(50, 'Too many required skills')
			.optional(),
		preferredSkills: z
			.array(sanitizedTextSchema(100))
			.max(50, 'Too many preferred skills')
			.optional(),
	})
...
export function validateBatchSecure<T>(
	...
	return dataArray.map((item, index) => {
		try {
			...
			throw new Error(
				`Validation failed for ${fieldName || 'item'} at index ${index}: ${errorMessage}`,
			)
		}
	})
}
