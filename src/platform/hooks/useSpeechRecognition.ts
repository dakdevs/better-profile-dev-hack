		...
		recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
			...
			switch (event.error) {
				...
				case 'network':
					...
					if (isBrave) {
						errorMessage =
							'Brave browser is blocking speech recognition. Click the Brave Shields icon (🦁) in your address bar and set "Block fingerprinting" to "Allow all fingerprinting", then refresh the page.'
					} else {
						errorMessage =
