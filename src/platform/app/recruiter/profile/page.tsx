...
export default function RecruiterProfilePage() {
	...
	const loadProfile = async () => {
		try {
			...
			console.log('[RECRUITER-PROFILE-PAGE] API response:', {
	...
	const handleCreateProfile = async (data: CreateRecruiterProfileRequest) => {
		try {
			...
			console.log('[RECRUITER-PROFILE-PAGE] Create profile response:', {
	...
	const handleDeleteProfile = async () => {
		if (
			!confirm(
				'Are you sure you want to delete your recruiter profile? This action cannot be undone.',
			)
		) {
			return
		}
	...
	return (
		<div className="space-y-8">
			...
			{/* Success Message */}
			{successMessage && (
				<div className="border-apple-green bg-apple-green/10 text-apple-green flex items-start gap-2 rounded-lg border px-4 py-3">
			...
			{process.env.NODE_ENV === 'development' && (
				<div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20">
					...
					<button
						...
						className="rounded bg-yellow-100 px-3 py-1 text-xs text-yellow-800 hover:bg-yellow-200 dark:bg-yellow-800 dark:text-yellow-200 dark:hover:bg-yellow-700"
					>
						Debug User Session
					</button>
				</div>
