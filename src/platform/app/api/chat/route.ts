...
import { db } from '~/db'
import { embeddings, interviewSessions, skillMentions, user, userSkills } from '~/db/schema'
import { InterviewRAGAgent } from '~/services/rag-agent'
...
function updateTopicTree(
	...
	if (hasExhaustionSignals || analysis.engagementLevel === 'low') {
		...
		if (currentNode.children.length > 0) {
			...
			console.log(
				`🔍 Going deeper: ${state.currentPath.map((id) => state.topicTree.get(id)?.name).join(' → ')}`,
			)
		}
	}
...
function generateSummaryTree(sessionId: string) {
	...
	const summary = {
		...
		topicCoverage: {
			explored: Array.from(state.topicTree.values()).filter((n) => n.status !== 'unexplored')
				.length,
			rich: Array.from(state.topicTree.values()).filter((n) => n.status === 'rich').length,
			exhausted: Array.from(state.topicTree.values()).filter((n) => n.status === 'exhausted')
				.length,
		},
	...
	console.log(
		`📈 Coverage: ${summary.topicCoverage.explored} explored, ${summary.topicCoverage.rich} rich, ${summary.topicCoverage.exhausted} exhausted`,
	)
...
async function upsertUserSkill(
	...
	const userSkillId = `${userId}_${norm
		.toLowerCase()
		.replace(/\s+/g, '_')
		.replace(/[^a-z0-9_-]/g, '')}`
	...
	try {
		...
		if (existing) {
			...
			const newAvgConfidence =
				(currentAvgConfidence * (newMentionCount - 1) + confidence) / newMentionCount
			...
			const engagementScore =
				engagementLevel === 'high' ? 1.0 : engagementLevel === 'medium' ? 0.7 : 0.4
			const frequencyScore = Math.min(newMentionCount / 10, 1.0) // Max out at 10 mentions
			const proficiencyScore = Math.round(
				(newAvgConfidence * 0.4 + engagementScore * 0.4 + frequencyScore * 0.2) * 100,
			)
			...
			console.log(
				`✅ User skill updated: ${updated[0].skillName} (mentions: ${newMentionCount}, proficiency: ${proficiencyScore})`,
			)
			return updated[0].id
		} else {
			...
			const proficiencyScore = Math.round(
				(confidence * 0.4
					+ (engagementLevel === 'high' ? 1.0 : engagementLevel === 'medium' ? 0.7 : 0.4) * 0.4
					+ 0.1 * 0.2)
					* 100,
			)
			...
			console.log(
				`✅ New user skill created: ${inserted[0].skillName} (proficiency: ${proficiencyScore})`,
			)
...
async function extractSkillsFromText(
	text: string,
): Promise<Array<{ skill: string; evidence: string; confidence: number }>> {
	try {
...
function extractSkillsFromTextFallback(
	text: string,
): Array<{ skill: string; evidence: string; confidence: number }> {
	const knownSkills = [
		'react',
		'reactjs',
		'typescript',
...
export async function POST(req: NextRequest) {
	try {
		...
		if (!userId) {
			return NextResponse.json(
				{ reply: 'Auth Error: User not found the auth logic is not implemented' },
				{ status: 401 },
			)
		}
		...
		const exhaustedTopics = state.exhaustedTopics
			.map((id) => state.topicTree.get(id)?.name)
			.join(', ')

		// Create dynamic system prompt with current state
		const dynamicPrompt = INTERVIEW_SYSTEM_PROMPT.replace('{topicTreeState}', topicTreeState)
		...
		after(async () => {
			...
			try {
				...
				console.log(
					`💬 RESPONSE: "${latestUserMessage.content.substring(0, 80)}${latestUserMessage.content.length > 80 ? '...' : ''}"`,
				)
				...
				const avgScore =
					state.grades.reduce((sum, g) => sum + g.score, 0) / state.grades.length || 0
