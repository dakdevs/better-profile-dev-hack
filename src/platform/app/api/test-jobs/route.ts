		...
		return NextResponse.json({
			...
			data: {
				...
				summary: {
					...
					avgRequiredSkills: jobDetails
						? jobDetails.reduce((sum, job) => sum + (job.requiredSkills?.length || 0), 0)
							/ jobDetails.length
						: 0,
					avgPreferredSkills: jobDetails
						? jobDetails.reduce((sum, job) => sum + (job.preferredSkills?.length || 0), 0)
							/ jobDetails.length
						: 0,
					avgExtractedSkills: jobDetails
						? jobDetails.reduce((sum, job) => sum + (job.extractedSkills?.length || 0), 0)
							/ jobDetails.length
						: 0,
					avgConfidenceScore: jobDetails
						? jobDetails.reduce((sum, job) => sum + (job.aiConfidenceScore || 0), 0)
							/ jobDetails.length
						: 0,
				},
			},
