...
export const candidateAvailability = pgTable(
	...
	(table) => [index('candidate_availability_user_idx').on(table.userId)],
)
...
export const interviewSessionsScheduled = pgTable(
	...
	(table) => [
		index('scheduled_interviews_job_idx').on(table.jobPostingId),
		index('scheduled_interviews_candidate_idx').on(table.candidateId),
		index('scheduled_interviews_recruiter_idx').on(table.recruiterId),
	],
)
